...
function main {
   # Set Jira user and group
    ...
    if [ ! -d ${JIRA_INSTALL_DIR}/.install4j ] || [ ! -d ${JIRA_HOME}/data ]; then
        ...
        cp -pr /usr/share/atlassian/jira/driver/mysql-connector-java-${MYSQL_CONNECTOR_JAVA_VERSION}-bin.jar ${JIRA_INSTALL_DIR}/lib

        # Change ownership of the Java Mysql connector
        chown ${JIRA_USER}:${JIRA_GROUP} ${JIRA_INSTALL_DIR}/lib/mysql-connector-java-${MYSQL_CONNECTOR_JAVA_VERSION}-bin.jar
